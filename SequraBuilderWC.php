...
//            'provider' => self::notNull($carrierInfos[0]),
    ...
    {
		if($this->_order->id)
        	return $this->_order->getContents();
    ...
    public function handlingItems()
    {
        ...
        $handling = array(
            ...
            'tax_rate' => self::integerPrice($this->_cart->shipping_tax_total / $this->_cart->shipping_total),
        );
        if ($delivery['days'])
    ...
    {
        $data = array();
        $data['given_names'] = self::notNull($this->_order->get_shipping_first_name());
        $data['surnames'] = self::notNull($this->_order->get_shipping_last_name());
        $data['company'] = self::notNull($this->_order->get_shipping_company());
        $data['address_line_1'] = self::notNull($this->_order->get_shipping_address_1());
        $data['address_line_2'] = self::notNull($this->_order->get_shipping_address_2());
        $data['postal_code'] = self::notNull($this->_order->get_shipping_postcode());
        $data['city'] = self::notNull($this->_order->get_shipping_city());
        $data['country_code'] = self::notNull($this->_order->get_shipping_country());
        // OPTIONAL
        $data['state'] = self::notNull($this->_order->get_shipping_state());
        $data['mobile_phone'] = self::notNull($this->_order->get_shipping_phone());
        $data['vat_number'] = self::notNull($this->_order->get_shipping_nif());
        if ('' == $data['vat_number'])
            $data['vat_number'] = self::notNull($this->_order->get_shipping_vat());
        return $data;
    }
    ...
    {
        $data = array();
        $data['given_names'] = self::notNull($this->_order->get_billing_first_name());
        $data['surnames'] = self::notNull($this->_order->get_billing_last_name());
        $data['company'] = self::notNull($this->_order->get_billing_company());
        $data['address_line_1'] = self::notNull($this->_order->get_billing_address_1());
        $data['address_line_2'] = self::notNull($this->_order->get_billing_address_2());
        $data['postal_code'] = self::notNull($this->_order->get_billing_postcode());
        $data['city'] = self::notNull($this->_order->get_billing_city());
        $data['country_code'] = self::notNull($this->_order->get_billing_country());
        // OPTIONAL
        $data['state'] = self::notNull($this->_order->get_billing_state());
        $data['mobile_phone'] = self::notNull($this->_order->get_billing_phone());
        $data['vat_number'] = self::notNull($this->_order->get_billing_nif());
        if ('' == $data['vat_number'])
            $data['vat_number'] = self::notNull($this->_order->get_billing_vat());
        return $data;
    }
    ...
    public function customer()
    {
		$data = array();
		if($this->_order->id)
			$customer_id = get_post_meta($this->_order->id, '_customer_user', true );
		else
			$customer_id = get_current_user_id();
		$customer = get_user_meta($customer_id);

		$data['given_names'] = self::notNull($customer->first_name);
		$data['surnames'] = self::notNull($customer->last_name);
		$data['email'] = self::notNull($customer->billing_email);
		$data['logged_in'] = is_user_logged_in();
		// OPTIONAL
		$data['date_of_birth'] = self::dateOrBlank($customer->dob);
		$data['company'] = self::notNull($customer->billing_company);
		$data['ref'] = $customer_id;
		$data['previous_orders'] = self::getPreviousOrders($customer_id);
