...
class SequraBuilderWC extends SequraBuilderAbstract {
	...
	public function merchant() {
		...
		if (isset($this->_pm) && is_object($this->_pm))
	...
	private function getShippingMethodFromSession() {
		...
		if (!$shipping_methods) $shipping_methods = array();
	...
	public function items() {
		...
		foreach ($cart_contents as $cart_item_key => $cart_item) {
			...
			if (isset($cart_item['quantity'])) $item["quantity"] = (int)$cart_item['quantity'];
			if (isset($cart_item['qty'])) $item["quantity"] = (int)$cart_item['qty'];
