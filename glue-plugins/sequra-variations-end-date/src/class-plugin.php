...
class Plugin {
	...
	public function init(): void {
		if ( ! class_exists( 'SeQura\Core\Infrastructure\ServiceRegister' ) ) {
			$this->show_error( 'seQura - Variations End Date requires seQura to be installed and active.' );
			return;
		}

		\SeQura\WC\VariationsEndDate\Bootstrap::init();

		$this->plugin_data     = (array) ServiceRegister::getService( 'plugin.data' );
		$this->addon_data      = (array) ServiceRegister::getService( 'variations_end_date_addon.data' );
		$this->addon_base_name = strval( ServiceRegister::getService( 'variations_end_date_addon.basename' ) );
