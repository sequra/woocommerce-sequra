...
<div id="sq_pm_<?php echo esc_html( $sq_product_campaign );?>" class="sq_payment_method <?php echo esc_html( $sq_product_campaign ); ?>">
	<input type="radio" name="sq_product_campaign" value="<?php echo esc_html( $sq_product_campaign );?>" id="sq_product_campaign_<?php echo esc_attr( $sq_product_campaign );?>" class="input-radio sq-input-radio"/>
	<label for="sq_product_campaign_<?php echo esc_html( $sq_product_campaign );?>" class="sq_payment_method">
		...
		<div class="sq_payment_method_title_claim">
			...
			<?php if ( !in_array($method['product'],['fp1']) ) { ?>
				...
				data-amount="<?php echo esc_attr((int)$this->get_order_total()*100); ?>" data-product="<?php echo esc_attr( $method['product'] );?>" data-campaign="<?php echo esc_attr( $method['campaign'] );?>"
	...
	<div class="sq_payment_method_cost">
		<?php if ( isset($method['cost_description']) ) { ?>
			<span id="sequra_cost_link_<?php echo esc_attr($sq_product_campaign); ?>" class="sequra-educational-popup sequra_cost_description"
				data-amount="<?php echo esc_attr((int)$this->get_order_total()*100); ?>" data-product="<?php echo esc_attr($method['product']); ?>" data-campaign="<?php echo esc_attr($method['campaign']); ?>"
