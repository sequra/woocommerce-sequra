...
?>
<div id="sq_pm_<?php esc_attr_e( $sq_product_campaign ); ?>"
	class="sq_payment_method <?php esc_attr_e( $sq_product_campaign ); ?>">
	<input type="radio" name="sq_product_campaign" value="<?php esc_attr_e( $sq_product_campaign ); ?>"
		id="sq_product_campaign_<?php esc_attr_e( $sq_product_campaign ); ?>" class="input-radio sq-input-radio" />
		...
		<div class="sq_payment_method_title_claim">
			...
			<?php if ( ! in_array( $method['product'], array( 'fp1' ) ) ) { ?>
				<span id="sequra_info_link" class="sequra-educational-popup sequra_more_info"
					data-amount="<?php esc_attr_e( (int) $this->get_order_total() * 100 ); ?>"
					data-product="<?php esc_attr_e( $method['product'] ); ?>"
					data-campaign="<?php esc_attr_e( $method['campaign'] ); ?>" rel="sequra_invoice_popup_checkout"
	...
	<div class="sq_payment_method_cost">
		<?php if ( isset( $method['cost_description'] ) ) { ?>
			<span id="sequra_cost_link_<?php esc_attr_e( $sq_product_campaign ); ?>"
